...
3. ［基準となる範囲］のメニューから、整列の基準としたい対象を選択します。`アートボード`、`選択範囲`、`最背面オブジェクト`、`最前面オブジェクト`の4つから選択可能です
...
スクリプトは最後に実行したダイアログの状態を記憶しますので、スクリプトを起動したあとにすぐreturn（enter）キーを押せば前回と同じ設定で整列を実行できます。さらに、`option（Alt）`キーを押しながらスクリプトを起動することで、ダイアログを省略しつつ前回と同じ整列を実行可能です。
...
通常の実行でもダイアログ表示をカットするようにカスタマイズできます。この場合は、あらかじめ設定した値に基づいて整列が実行されます。スクリプトをテキストエディタで開き、10行目あたりにある`settings`の各値を変更してください。ダイアログ表示をカットする場合の整列位置や基準は、ここで設定した値が使われます。
...
| previewArea | Boolean | 整列基準範囲のハイライト表示するか | false：しない｜true：する |
| showDialog | Boolean | ダイアログを表示するか | false：しない｜true：する |

* ダイアログ表示をカットしたい場合は、`showDialog`を`false`にしてください
* ダイアログ表示をカット時`previewArea`の値は使われません
...
* CS5、CS6、CC 2015.3、CC 2017（いずれもMac版）で動作の確認はしましたが、OSのバージョンやその他の状況によって実行できないことがあるかもしれません。もし動かなかったらごめんなさい。
